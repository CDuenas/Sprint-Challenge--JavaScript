...
function getUniversities(array) {
  const universities = [];
  for (let i = 0; i < array.length; i++){
      universities.push(array[i].first_name + " " + array[i].email);
    }
    return universities;
}
...
function getUniversities(array) {
  ...
  for (let i = 0; i < array.length; i++){
    ...
    return unisWithUni.filter(function(item){
        var finder = 'Uni';
...
function getFullNames(zooAnimals) {
  ...
  zooAnimals.forEach((zooAnimal) => {
    displayNames.push(zooAnimal.animal_name + ", " + zooAnimal.scientific_name);
  });
  return displayNames;
}

console.log(getFullNames(zooAnimals));
/* Request 2: .map()
...
console.log(lowCase(zooAnimals));
...
*/
function countPopulation(zooAnimals) {
  const populationTotal = zooAnimals.reduce((accum, animal) => {
    return accum + animal.population;
  }, 0);
  return populationTotal;
};

console.log(countPopulation(zooAnimals));
